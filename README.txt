...
Пример запуска:
python linker.py -n 15000

Объекты:

Docs - хранит информацию о структуре файлов.

Document - хранит информацию о документе. 
...
Link - хранит информацию о ссылке, которая есть в исходной разметке (нужно было для аналитики, можно убрать из работающей системы).
В закоммиченном варианте код, связанный с обработкой существующих ссылок, не вызывается.

Index - объект, который запускает основные действия
1) добавить в индекс n документов из коллекции
2) (отключено) походить по всем ссылкам и проанализировать их
3) походить по всем документам, найти пары "документ - изменения и дополнения к документу", найти в "изменениях и дополнениях" собственно тексты модификаций и вставить на соответствующие абзацы ссылки в основной документ
